...
test('Automate ZFW Hospitality Brand Configuration', async ({ page }) => {
  ...
  // 9b. Make sure we are on the Standard Delivery module
  console.log('Ensuring Standard Delivery module is selected...');
  try {
    // Click it to be absolutely sure it's the active view
    await page.getByText('Standard Delivery', { exact: true }).click({ timeout: 3000 });
    await page.waitForTimeout(1000);
  } catch (e) {
    console.log('Standard Delivery already selected or not clickable as a tab.');
  }
  ...
  // --- Pickup ---
  await setDropdownOption("AWB Scan", getOption('PICKUP', 0), 0);
  await setDropdownOption("Proof of Pickup (photo)", getOption('PICKUP', 1), 0);
  await setDropdownOption("OTP", getOption('PICKUP', 2), 0);

  // --- Delivered ---
  await setDropdownOption("AWB Scan", getOption('DELIVERED', 0), 1);
  await setDropdownOption("Proof of Delivery Image Capture", getOption('DELIVERED', 1), 0);
  await setDropdownOption("OTP For COD", getOption('DELIVERED', 2), 0);
  await setDropdownOption("OTP For Prepaid", getOption('DELIVERED', 3), 0);
  await setDropdownOption("Partial Delivery", process.env.DELIVERED_PARTIAL_DELIVERY, 0);

  // --- Return Pickup ---
  await setDropdownOption("Proof of Pickup (photo)", getOption('RETURN_PICKUP', 0), 1);
  await setDropdownOption("Quality Check 1", getOption('RETURN_PICKUP', 0), 0);
  await setTextInput("Question for Quality Check 1", process.env.RETURN_PICKUP_QC_1_QUESTION?.replace(/^"|"$/g, ''), 0);
  await setDropdownOption("Quality Check 2", getOption('RETURN_PICKUP', 0), 0);
  await setTextInput("Question for Quality Check 2", process.env.RETURN_PICKUP_QC_2_QUESTION?.replace(/^"|"$/g, ''), 0);
  await setDropdownOption("Quality Check 3", getOption('RETURN_PICKUP', 0), 0);
  await setTextInput("Question for Quality Check 3", process.env.RETURN_PICKUP_QC_3_QUESTION?.replace(/^"|"$/g, ''), 0);
  await setDropdownOption("OTP For Prepaid", getOption('RETURN_PICKUP', 0), 1);
  await setDropdownOption("OTP For COD", getOption('RETURN_PICKUP', 0), 1);
  await setDropdownOption("Remarks", getOption('RETURN_PICKUP', 0), 0);
  await setDropdownOption("Item Level Scanning", getOption('RETURN_PICKUP', 0), 0);
  await setDropdownOption("Item wise separate return shipment", process.env.RETURN_PICKUP_ITEM_WISE_SEPARATE, 0);

  // --- Return Delivery ---
  await setDropdownOption("AWB Scan", getOption('RETURN_DELIVERY', 0), 3);
  await setDropdownOption("Proof of Delivery", getOption('RETURN_DELIVERY', 1), 0);
  await setDropdownOption("OTP", getOption('RETURN_DELIVERY', 2), 1);
  ...
  console.log('Brand Configuration automated successfully!');
});
